#!/usr/bin/env python3
"""Train one linear projection versus completed two-linear no-position controls."""
import argparse
import fcntl
import hashlib
import json
...
from birdsong_detect_distill.pointwise_single_linear import ARCHITECTURE, SingleLinearHead, load_heads
from prepare_detector_study import verify
from run_pointwise_no_position_2k import OUT as PARENT, ARTIFACTS as PARENT_ARTIFACTS

RUN = 'pointwise_single_linear_2000train_800val_2026-09-14'
OUT, ARTIFACTS = PARENT.parent / RUN, PARENT_ARTIFACTS.parent / RUN
SCRIPT = 'scripts/run_pointwise_single_linear_2k.py'
MODEL = 'src/birdsong_detect_distill/pointwise_single_linear.py'
...
def prepare():
    ...
    if json.loads((PARENT / 'status.json').read_text())['state'] != 'complete':
        raise ValueError('no-position controls must be complete')
    protected, controls = dict(parent['protected_files']), []
    for seed in parent['seeds']:
        path = PARENT / 'conditions' / f'no_position_d128_new2k_s{seed}.json'
    ...
    for p in [PARENT / 'manifest.json',PARENT / 'status.json',PARENT / 'summary.json']:
        protected[str(p)] = digest(p)
    plan = {**parent,'run':RUN,'widths':[0],'controls':controls,'artifacts':str(ARTIFACTS),
        'protected_files':protected,'code_sha256':{**parent['code_sha256'],SCRIPT:digest(SCRIPT),MODEL:digest(MODEL)},
        'architecture':'LayerNorm(768) -> Linear(768,32); no positions, attention, GELU or hidden projection',
        'ablation':'One affine layer versus two; same initial function by fusing randomly initialized control weights',
        'initialization':'W=W2@W1; b=W2@b1+b2; NOT initialized from trained control weights',
        'pairing':'Same initial function up to floating-point rounding; same LayerNorm, sample order and CPU RNG',
        'hidden_metadata':'0 is a sentinel for no hidden projection; actual input/output dimensions are 768/32',
        'head_parameters':26144,'control_head_parameters':104096,
        'schedule':'Twins only; preserve V100 annotations and leave Twins Qwen paused'}
    verify(plan)
...
def check_pair(saved, control, audit):
    ...
    if audit['control_initial_sha256'] != old['initial_head_sha256'] or audit['single_initial_sha256'] != saved['initial_head_sha256']:
        raise ValueError('initialization is not matched to the no-position control')
    if set(saved['head']) != {'norm.weight','norm.bias','output.weight','output.bias'} or sum(v.numel() for v in saved['head'].values()) != 26144:
        raise ValueError('unexpected single-linear architecture')
...
def train(spec, plan):
    ...
    SingleLinearHead(768,0,4,1000,32,1,layers=0).load_state_dict(saved['head'])
    saved.update(detector_architecture=ARCHITECTURE,head_activation='none',detector_position_embeddings=False,
        layer_norm=True,hidden_projection=None,initialization_audit=audit,
    ...
    print(f'Validated and tagged single-linear checkpoint: {target}',flush=True)
...
def run(plan):
    ...
    specs = [shared.study.condition('single_linear',0,seed,dataset='new2k',layers=0) for seed in plan['seeds']]
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        for future in futures:
            future.result()
    groups = {}
    for name,conditions in [('two_linear',plan['controls']),('single_linear',specs)]:
    ...
    deltas = [dict(seed=a['seed'],**{k:float(b[k]-a[k]) for k in ['ap','iou']}) for a,b in
        zip(groups['two_linear']['per_seed'],groups['single_linear']['per_seed'])]
    verify(plan)
    write_json(OUT / 'summary.json',dict(groups=groups,paired_single_minus_two_linear=deltas,
...
def main():
    ...
    try:
        ...
        if child:
            ...
            print('Frozen: three single-linear runs, 2,000 s train + 800 s validation, no-position controls.')
        else:
            run(plan)
