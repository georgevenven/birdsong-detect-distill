...
run=large_predictor_width_10000train_800val_2026-09-10
...
.venv/bin/python scripts/summarize_large_predictor_width.py --prepare
.venv/bin/python - <<'PY'
...
if not (Path.home() / '.config/birdsong-detect-distill/qwen.paused').exists():
    raise SystemExit('Expected Qwen manual-pause marker is absent.')
print('GPU and disk checks passed; Qwen stays paused.', flush=True)
PY
if [[ ! -f "$artifacts/large_d384.pt" ]]; then
  .venv/bin/python -u scripts/train.py \
    ...
    --hidden 384 --dropout 0.1 --weight-decay 0.0001 --tv-weight 0 --no-target-smoothing \
    --log-every 25 --out "$artifacts/large_d384.pt" \
    2>&1 | tee -a "$results/logs/train_large_d384.log"
fi
.venv/bin/python -u scripts/evaluate_current_backbones.py --size large --seconds 10000 --seed 0 \
  --training-epochs 5 --head-width 384 --fixed-large-width --prediction-cache samples \
  2>&1 | tee -a "$results/logs/evaluate_large_d384.log"
.venv/bin/python scripts/summarize_large_predictor_width.py
