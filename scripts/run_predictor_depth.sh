...
run=predictor_depth_10000train_800val_2026-09-10
...
.venv/bin/python scripts/summarize_predictor_depth.py --prepare
.venv/bin/python - <<'PY'
...
PY
if [[ ! -f "$artifacts/large_d384_l2.pt" ]]; then
  .venv/bin/python -u scripts/train.py \
    ...
    --hidden 384 --head-layers 2 --dropout 0.1 --weight-decay 0.0001 --tv-weight 0 --no-target-smoothing \
    --log-every 25 --out "$artifacts/large_d384_l2.pt" \
    2>&1 | tee -a "$results/logs/train_large_d384_l2.log"
fi
.venv/bin/python -u scripts/evaluate_current_backbones.py --size large --seconds 10000 --seed 0 \
  --training-epochs 5 --head-width 384 --head-layers 2 --fixed-large-width --prediction-cache samples \
  2>&1 | tee -a "$results/logs/evaluate_large_d384_l2.log"
.venv/bin/python scripts/summarize_predictor_depth.py
