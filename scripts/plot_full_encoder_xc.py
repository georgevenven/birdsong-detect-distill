#!/usr/bin/env python3
"""Full-encoder fine-tuning on the same eight held-out XC figure excerpts."""
import hashlib
import json
...
OUT = base.ROOT / 'results/full_encoder_xc_heldout_2026-09-14'
STUDY = base.ROOT / 'results/qwen_teacher_powdermill/full_encoder_linear_2000train_800val_2026-09-14'
REPORT = STUDY / 'comparison.json'
...
def prepare():
    ...
    protocol, saved = report['protocol'], report['protocol']['checkpoint']
    control = json.loads((base.STUDY / 'runs/bare_linear_d0_new2k_s0/comparison.json').read_text())['protocol']['checkpoint']
    for key in ['seed','training_recording_ids','validation_recording_ids','backbone_id',
        ...
        if saved[key] != control[key]:
            raise ValueError(f'comparison differs in {key}')
    if (saved['detector_architecture'] != ARCHITECTURE or saved['metrics']['train_timebins'] != 400000
            or base.digest(protocol['checkpoint_path']) != protocol['checkpoint_sha256']):
        raise ValueError('unexpected or changed full-encoder checkpoint')
    plan = {**original, 'checkpoint':protocol['checkpoint_path'], 'threshold':report['summary']['threshold'],
        'architecture':'Full SongMAE-Large detection encoder fine-tuned with a bare linear head',
        'checkpoint_metadata':saved, 'calibration_report':str(REPORT),
        'threshold_source':'Original Powdermill calibration Recordings_2–4; not selected on these XC examples',
        'protected':{**original['protected'], **study['code_sha256']}}
    paths = [Path(__file__), REPORT, STUDY / 'manifest.json', Path(plan['checkpoint']),
        base.ROOT / 'src/birdsong_detect_distill/full_encoder_linear.py',
        base.OUT / 'manifest.json', base.OUT / 'examples.json', base.OUT / 'share/README.md']
    plan['protected'].update({str(p):base.digest(p) for p in paths})
    path = OUT / 'manifest.json'
    if path.exists() and json.loads(path.read_text()) != plan:
        raise ValueError('frozen figure dependencies changed')
...
def panels(axes, entry, display, mask, overview=False):
    ...
    axes[1].set_title('(b) Full fine-tuning, post-processed', pad=6)
...
def main():
    ...
    backbone = base.load_backbone(plan['backbone'], device, revision=plan['revision'])
    ...
    overview.suptitle('Held-out XC · SongMAE-Large, full encoder fine-tuning + linear head · 2,000 s training\n'
    ...
    for i, (entry, previous) in enumerate(zip(plan['examples'], originals, strict=True)):
        ...
        if source_hash != previous['full_source_spectrogram_sha256']:
            raise ValueError('input differs from the previous figures')
    ...
    notes = ['# Held-out XC: fully fine-tuned SongMAE-Large', '',
        ...
        'Entire detection encoder fine-tuned with Linear(768,32), 96,493,856 trainable parameters. Seed 0; 2,000 s training and 800 s separate XC validation; best validation-BCE checkpoint (epoch 5). No new training.',
        ...
        'Identical recordings, excerpts, inputs, normalization, full-source inference/overlap, smoothing and display as the earlier figures. The frozen linear model used threshold 0.07; each model uses its own pre-calibrated threshold.',
        plan['inference'] + '.', plan['selection'] + '.', plan['exclusions'] + '.',
        'Qualitative examples only; no human localization ground truth or benchmark metrics.', '',
    ...
    base.write_json(share / 'provenance.json', dict(checkpoint=plan['checkpoint_metadata'],
        checkpoint_sha256=base.digest(checkpoint), threshold=plan['threshold'],
        manifest_sha256=base.digest(OUT / 'manifest.json'), examples=entries))
    if any(base.digest(p) != sha for p,sha in plan['protected'].items()):
        raise ValueError('figure inputs changed during rendering')
