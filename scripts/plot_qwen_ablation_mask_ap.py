...
def main():
    parser = argparse.ArgumentParser(description="Plot Qwen pipeline ablation 2D mask AP.")
    ...
    fig, axis = plt.subplots(figsize=(5.8, 4.5), dpi=200)
    scores = []
    for variant, (label, color) in VARIANTS.items():
        points = sorted((int(amount), result["mask_ap_2d"])
            for name, amount, result in values if name == variant)
        x, y = zip(*points)
        scores.extend(y)
        axis.plot(x, y, marker="o", markersize=6, linewidth=2, color=color, label=label)
    margin = max(.005, (max(scores) - min(scores)) * .15)
    axis.set_xscale("log")
    ...
    axis.set_ylabel("Mean 2D mask AP ↑")
    axis.set_title("Qwen annotation pipeline ablation")
    axis.grid(alpha=.18)
