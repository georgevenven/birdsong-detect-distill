#!/usr/bin/env python3
"""Scaling study: new default recipe at 250/1k/2.5k/5k/10k/25k s, micro/base/large."""
import argparse
import fcntl
import hashlib
import os
...
JOBS = {f'{size}_{budget}': dict(size=size, budget=budget)
    for size in SIZES for budget in BUDGETS}
...
def prepare():
    ...
    for size in ['micro', 'base', 'large']:
        ...
        initializations[size] = dict(
            initial_head_sha256=state_hash(head.state_dict()),
            initial_encoder_sha256=state_hash(encoder_state(backbone)),
            encoder_depth=len(backbone.songmae.encoder.layers))
        del backbone, head
    if initializations['large']['initial_encoder_sha256'] != 'd6c687d829ca99c8b2e37b2e2710e3e1827ede991f0e56ff3d3416ec9d2cadf1':
        raise ValueError('Pretrained Large encoder differs')
    if not initializations['base']['initial_encoder_sha256'].startswith('59284e98425daf1a'):
        raise ValueError('Pretrained Base encoder differs')
    plan = dict(run='scaling_default_2026-09-18', seed=0, epochs=5,
...
def worker(job):
    ...
    plan['initialization'] = master['initializations'][plan['size']]
    ...
    try:
        ...
        best_epoch = read(out / 'checkpoint.json')['metrics']['selected_epoch']
        write(out / 'result.json', dict(job=job, size=plan['size'], budget=plan['recipe']['budget'],
...
def driver():
    ...
    try:
        ...
        lines = ['# Scaling — new default, 5 epochs, best XC BCE', '',
            '| Budget (s) | Micro AP | Micro IoU | Base AP | Base IoU | Large AP | Large IoU |',
            '|---:|---:|---:|---:|---:|---:|---:|']
        by = {(r['size'], r['budget']): r for r in rows}
        for budget in BUDGETS:
            cells = [str(budget)]
            for size in SIZES:
                r = by[(size, budget)]
...
if __name__ == '__main__':
    if platform.node() != 'Lambda-Twins' or ROOT != Path('/mnt/birdconv/songmae_perch2/birdsong_detect_distill_experiments/scaling-default-20260918'):
