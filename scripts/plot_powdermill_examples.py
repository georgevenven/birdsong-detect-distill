...
def draw_example(raw, probability, events, threshold, path):
    fig, axes = plt.subplots(2, 1, sharex=True, sharey=True, figsize=(3.5, 3.5))
    fig.subplots_adjust(left=.17, right=.98, bottom=.14, top=.92, hspace=.40)
    extent = (0, 5, 0, 128)
    color = "#00D5FF"
    for axis in axes:
    ...
    axes[1].set(title="(b) SongMAE-Large (10k s)", xlabel="Time (s)")
...
def main():
    ...
    parser.add_argument("--out", type=Path, default=Path("results/qwen_teacher_powdermill/powdermill_examples_2026-09-07"))
    parser.add_argument("--cache", type=Path, default=Path("artifacts/powdermill_examples_2026-09-07"))
    ...
    if args.render_only:
        ...
        report = {"comparison": str(args.comparison), "comparison_sha256": digest(args.comparison),
            ...
            "inference": comparison["student_inference"], "visible_devices": os.environ.get("CUDA_VISIBLE_DEVICES"),
            ...
            "display": {"seconds": 5, "frequency_scale": "mel", "frequency_hz": [20, 16000],
                "spectrogram_db": [-70, 0], "reference": "full_segment_maximum", "postprocessing": "none"},
            "examples": []}
        ...
        for recording, waveform, events in powdermill(args.root, selected):
            ...
            for identifier, description, segment, start in EXAMPLES:
                ...
                np.savez_compressed(cache, raw=spec, probability=prob, events=cropped)
                report["examples"].append({"id": identifier, "description": description, "recording": recording,
                    ...
                    "clip_metrics": score_masks(prob, prob >= threshold, truth)})
    ...
    for row in report["examples"]:
        ...
        with np.load(row["cache"]) as data:
            draw_example(data["raw"], data["probability"], data["events"], report["threshold"], args.out / row["file"])
